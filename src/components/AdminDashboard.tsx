...
export function AdminDashboard({ onLogout }: AdminDashboardProps) {
  ...
  const [formData, setFormData] = useState({ name: '', nationalCode: '', insuranceCode: '', phone: '', email: '' });
  ...
  const [formDataPolicy, setFormDataPolicy] = useState({ customerName: '', type: '', vehicle: '', startDate: '', endDate: '', premium: '', status: 'فعال', pdfFile: null as File | null });
  ...
  const [formDataInstallment, setFormDataInstallment] = useState({ customerName: '', policyType: '', amount: '', dueDate: '', payLink: '', status: 'معوق' });
  ...
  const handleAddCustomer = () => {
    ...
    setFormData({ name: '', nationalCode: '', insuranceCode: '', phone: '', email: '' });
    setIsAddDialogOpen(false);
  };

  const handleEditCustomer = () => {
    if (!editingCustomer) return;
    setCustomers(customers.map(c => c.id === editingCustomer.id ? { ...c, name: formData.name, nationalCode: formData.nationalCode, phone: formData.phone, email: formData.email, password: formData.insuranceCode } : c));
    setFormData({ name: '', nationalCode: '', insuranceCode: '', phone: '', email: '' });
  ...
  const handleAddPolicy = () => {
    ...
    setFormDataPolicy({ customerName: '', type: '', vehicle: '', startDate: '', endDate: '', premium: '', status: 'فعال', pdfFile: null });
    setIsAddPolicyDialogOpen(false);
  };

  const handleEditPolicy = () => {
    if (!editingPolicy) return;
    setPolicies(policies.map(p => p.id === editingPolicy.id ? { ...p, ...formDataPolicy } : p));
    setFormDataPolicy({ customerName: '', type: '', vehicle: '', startDate: '', endDate: '', premium: '', status: 'فعال', pdfFile: null });
  ...
  const handleAddInstallment = () => {
    ...
    setFormDataInstallment({ customerName: '', policyType: '', amount: '', dueDate: '', payLink: '', status: 'معوق' });
    setIsAddInstallmentDialogOpen(false);
  };

  const handleEditInstallment = () => {
    if (!editingInstallment) return;
    setInstallments(installments.map(i => i.id === editingInstallment.id ? { ...i, ...formDataInstallment } : i));
    setFormDataInstallment({ customerName: '', policyType: '', amount: '', dueDate: '', payLink: '', status: 'معوق' });
  ...
  const filteredInstallments = installments.filter(i => 
    i.customerName.toLowerCase().includes(installmentSearchQuery.toLowerCase()) || 
  ...
  const handleAddBlog = () => {
    ...
    setFormDataBlog({ title: '', excerpt: '', content: '', author: '', date: '', imageUrl: '', category: '' });
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
                    <DialogTrigger asChild>
                      <Button onClick={() => setFormData({ name: '', nationalCode: '', insuranceCode: '', phone: '', email: '' })}>
                          ...
                          <div className="flex gap-2">
                            ...
                            <AlertDialog>
                              <AlertDialogTrigger asChild>
                                <Button size="sm" variant="outline" className="text-red-600 hover:text-red-700" onClick={() => setDeleteCustomer(customer)}>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    آیا مطمئن هستید که می‌خواهید این مشتری را حذف کنید؟ این عمل قابل بازگشت نیست.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                  <AlertDialogCancel>لغو</AlertDialogCancel>
                                  <AlertDialogAction onClick={handleDeleteCustomer} className="bg-red-600 hover:bg-red-700">حذف</AlertDialogAction>
                                </AlertDialogFooter>
                              </AlertDialogContent>
                            </AlertDialog>
                          </div>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Dialog open={isAddPolicyDialogOpen} onOpenChange={setIsAddPolicyDialogOpen}>
                    <DialogTrigger asChild>
                      <Button onClick={() => setFormDataPolicy({ customerName: '', type: '', vehicle: '', startDate: '', endDate: '', premium: '', status: 'فعال', pdfFile: null })}>
                        <Plus className="h-4 w-4 ml-2" />
                        صدور بیمه‌نامه
                      </Button>
                    </DialogTrigger>
                    <DialogContent className="bg-white/95 backdrop-blur-sm border shadow-lg">
                      <DialogHeader>
                        ...
                        <DialogDescription>اطلاعات بیمه‌نامه را وارد کنید</DialogDescription>
                      </DialogHeader>
                      <div className="grid gap-4 py-4">
                        <div className="grid grid-cols-4 items-center gap-4">
                      ...
                      <DialogFooter>
                        <Button onClick={handleAddPolicy}>صدور بیمه‌نامه</Button>
                      </DialogFooter>
                    </DialogContent>
                  </Dialog>
                </div>
                  ...
                  <TableBody>
                    {policies.map((policy) => (
                      <TableRow key={policy.id}>
                        ...
                        <TableCell>
                          <div className="flex gap-2">
                            <Button size="sm" variant="outline" onClick={() => openEditPolicyDialog(policy)}>
                              <Edit className="h-4 w-4" />
                            </Button>
                            <AlertDialog>
                              <AlertDialogTrigger asChild>
                                <Button size="sm" variant="outline" className="text-red-600 hover:text-red-700" onClick={() => setDeletePolicy(policy)}>
                                  <Trash2 className="h-4 w-4" />
                                </Button>
                              </AlertDialogTrigger>
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  <AlertDialogTitle>حذف بیمه‌نامه</AlertDialogTitle>
                                  <AlertDialogDescription>
                                    آیا مطمئن هستید که می‌خواهید این بیمه‌نامه را حذف کنید؟ این عمل قابل بازگشت نیست.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                  <AlertDialogCancel>لغو</AlertDialogCancel>
                                  <AlertDialogAction onClick={handleDeletePolicy} className="bg-red-600 hover:bg-red-700">حذف</AlertDialogAction>
                                </AlertDialogFooter>
                              </AlertDialogContent>
                            </AlertDialog>
                          </div>
              ...
              <Dialog open={isEditPolicyDialogOpen} onOpenChange={setIsEditPolicyDialogOpen}>
                <DialogContent className="bg-white/95 backdrop-blur-sm border shadow-lg">
                  <DialogHeader>
                    ...
                    <DialogDescription>اطلاعات بیمه‌نامه را ویرایش کنید</DialogDescription>
                  </DialogHeader>
                  <div className="grid gap-4 py-4">
                    <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Dialog open={isAddInstallmentDialogOpen} onOpenChange={setIsAddInstallmentDialogOpen}>
                    <DialogTrigger asChild>
                      <Button onClick={() => setFormDataInstallment({ customerName: '', policyType: '', amount: '', dueDate: '', payLink: '', status: 'معوق' })}>
                        <Plus className="h-4 w-4 ml-2" />
                        افزودن قسط
                      </Button>
                    </DialogTrigger>
                    <DialogContent className="bg-white/95 backdrop-blur-sm border shadow-lg">
                      <DialogHeader>
                        ...
                        <DialogDescription>اطلاعات قسط را وارد کنید</DialogDescription>
                      </DialogHeader>
                      <div className="grid gap-4 py-4">
                        <div className="grid grid-cols-4 items-center gap-4">
                      ...
                      <DialogFooter>
                        <Button onClick={handleAddInstallment}>افزودن قسط</Button>
                      </DialogFooter>
                    </DialogContent>
                  </Dialog>
                </div>
              ...
              <CardContent>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead onClick={() => handleSort('customerName')} className="cursor-pointer hover:bg-gray-50">نام مشتری</TableHead>
                      <TableHead onClick={() => handleSort('policyType')} className="cursor-pointer hover:bg-gray-50">نوع بیمه</TableHead>
                      <TableHead onClick={() => handleSort('amount')} className="cursor-pointer hover:bg-gray-50">مبلغ قسط</TableHead>
                      <TableHead onClick={() => handleSort('dueDate')} className="cursor-pointer hover:bg-gray-50">سررسید</TableHead>
                      <TableHead>تعداد روز تاخیر</TableHead>
                      <TableHead onClick={() => handleSort('status')} className="cursor-pointer hover:bg-gray-50">وضعیت</TableHead>
                      <TableHead>عملیات</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {sortedInstallments.map((installment) => (
                      <TableRow key={installment.id}>
                        ...
                        <TableCell>
                          <div className="flex gap-2">
                            <Button size="sm" variant="outline" onClick={() => openEditInstallmentDialog(installment)}>
                              <Edit className="h-4 w-4" />
                            </Button>
                            <AlertDialog>
                              <AlertDialogTrigger asChild>
                                <Button size="sm" variant="outline" className="text-red-600 hover:text-red-700" onClick={() => setDeleteInstallment(installment)}>
                                  <Trash2 className="h-4 w-4" />
                                </Button>
                              </AlertDialogTrigger>
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    آیا مطمئن هستید که می‌خواهید این قسط را حذف کنید؟ این عمل قابل بازگشت نیست.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                  <AlertDialogCancel>لغو</AlertDialogCancel>
                                  <AlertDialogAction onClick={handleDeleteInstallment} className="bg-red-600 hover:bg-red-700">حذف</AlertDialogAction>
                                </AlertDialogFooter>
                              </AlertDialogContent>
                            </AlertDialog>
                          </div>
              ...
              <Dialog open={isEditInstallmentDialogOpen} onOpenChange={setIsEditInstallmentDialogOpen}>
                <DialogContent className="bg-white/95 backdrop-blur-sm border shadow-lg">
                  <DialogHeader>
                    ...
                    <DialogDescription>اطلاعات قسط را ویرایش کنید</DialogDescription>
                  </DialogHeader>
                  <div className="grid gap-4 py-4">
                    <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <DialogFooter>
                    <Button onClick={handleEditInstallment}>ذخیره تغییرات</Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
            </Card>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Dialog open={isAddBlogDialogOpen} onOpenChange={setIsAddBlogDialogOpen}>
                    <DialogTrigger asChild>
                      <Button onClick={() => setFormDataBlog({ title: '', excerpt: '', content: '', author: '', date: '', imageUrl: '', category: '' })}>
                        <Plus className="h-4 w-4 ml-2" />
                        افزودن مقاله
                      </Button>
                    </DialogTrigger>
                    <DialogContent className="bg-white/95 backdrop-blur-sm border shadow-lg max-w-2xl">
                      <DialogHeader>
                        ...
                        <DialogDescription>اطلاعات مقاله را وارد کنید</DialogDescription>
                      </DialogHeader>
                  ...
                  <TableBody>
                    {filteredBlogs.map((blog) => (
                      <TableRow key={blog.id}>
                        ...
                        <TableCell>
                          <div className="flex gap-2">
                            <Button size="sm" variant="outline" onClick={() => openEditBlogDialog(blog)}>
                              <Edit className="h-4 w-4" />
                            </Button>
                            <AlertDialog>
                              <AlertDialogTrigger asChild>
                                <Button size="sm" variant="outline" className="text-red-600 hover:text-red-700" onClick={() => setDeleteBlogId(blog.id)}>
                                  <Trash2 className="h-4 w-4" />
                                </Button>
                              </AlertDialogTrigger>
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    آیا مطمئن هستید که می‌خواهید این مقاله را حذف کنید؟ این عمل قابل بازگشت نیست.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                  <AlertDialogCancel>لغو</AlertDialogCancel>
                                  <AlertDialogAction onClick={handleDeleteBlog} className="bg-red-600 hover:bg-red-700">حذف</AlertDialogAction>
                                </AlertDialogFooter>
                              </AlertDialogContent>
                            </AlertDialog>
                          </div>
              ...
              <Dialog open={isEditBlogDialogOpen} onOpenChange={setIsEditBlogDialogOpen}>
                <DialogContent className="bg-white/95 backdrop-blur-sm border shadow-lg max-w-2xl">
                  <DialogHeader>
                    ...
                    <DialogDescription>اطلاعات مقاله را ویرایش کنید</DialogDescription>
                  </DialogHeader>
                  <div className="grid gap-4 py-4 max-h-96 overflow-y-auto">
                    <div className="grid grid-cols-4 items-center gap-4">
